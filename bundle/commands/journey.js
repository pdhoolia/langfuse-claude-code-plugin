...
// dist/commands/journey.js
var SCORE_NAME = "session_feedback";
var USAGE_HINT = "Usage: /journey up|down [optional comment]";
...
async function main() {
  ...
  if (isLookupFailure(target)) {
    if (target.kind === "no_session") {
      ...
      console.log("No conversation yet to score. Try /journey after Claude has responded at least once.");
    }
    return;
  }
  ...
  try {
    ...
    debug(`score posted: id=${scoreId} name=${SCORE_NAME} value=${value > 0 ? "+1" : "-1"} sessionId=${target.sessionId}`);
