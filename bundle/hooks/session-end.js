...
async function closeInterruptedTurn(options) {
  ...
  return {
    lastLine,
    turnsTraced,
    finalizedTraceId: turnsTraced > 0 ? sessionState.current_trace_id : void 0
  };
}
...
function removeActiveByCwd(state, cwd, sessionId) {
  const map = state._active_by_cwd;
  if (!map || map[cwd] !== sessionId)
    return state;
...
async function main() {
  ...
  await atomicUpdateState(config.stateFilePath, (s) => removeActiveByCwd(s, realCwd, input.session_id));
  if (!sessionState.current_trace_id) {
    ...
    await atomicUpdateState(config.stateFilePath, (s) => {
      ...
      return {
        ...
        [input.session_id]: {
          ...
          last_substantive_trace_id: turnsTraced > 0 && finalizedTraceId ? finalizedTraceId : ss.last_substantive_trace_id,
          task_run_map: {},
          tool_start_times: {},
