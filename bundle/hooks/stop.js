...
function pruneOldSessions(state, now = Date.now()) {
  ...
  for (const [key, value] of Object.entries(state)) {
    if (RESERVED_KEYS.has(key))
      continue;
    if (!value || isReservedShape(value))
      continue;
    const session = value;
...
async function retriable(fn, props = {}, log2) {
  ...
  for (let i = 0; i < retryCount + 1; i++) {
    if (i > 0) {
      ...
      log2(`Retrying ${i + 1} of ${retryCount + 1}`);
    }
    try {
      const res = await fn();
      return res;
      ...
      log2(`Retriable error: ${JSON.stringify(e)}`);
    }
  }
  throw lastError;
}
...
var LangfuseMedia = class _LangfuseMedia {
  ...
  readFile(filePath) {
    try {
      ...
      console.error(`Error reading file at path ${filePath}`, error2);
      return void 0;
    }
  }
  parseBase64DataUri(data) {
    try {
      ...
      console.error("Error parsing base64 data URI", error2);
...
function isLangfuseFetchHttpError(error2) {
  return typeof error2 === "object" && error2.name === "LangfuseFetchHttpError";
}
function isLangfuseFetchNetworkError(error2) {
  return typeof error2 === "object" && error2.name === "LangfuseFetchNetworkError";
}
...
function logIngestionError(error2) {
  if (isLangfuseFetchHttpError(error2)) {
    const code = error2.response.status;
    const errorResponse = getErrorResponseByCode(code);
    console.error("[Langfuse SDK]", errorResponse, `Error details: ${error2}`);
  } else if (isLangfuseFetchNetworkError(error2)) {
    console.error("[Langfuse SDK] Network error: ", error2);
  } else {
    console.error("[Langfuse SDK] Unknown error:", error2);
  }
}
...
var LangfuseCore = class extends LangfuseCoreStateless {
  ...
  async _fetchPromptAndUpdateCache(params) {
    ...
    try {
      ...
      console.error(`[Langfuse SDK] Error while fetching prompt '${cacheKey}':`, error2);
      throw error2;
    }
  }
  async fetchMedia(id) {
...
var LangfuseObservationClient = class extends LangfuseObjectClient {
  constructor(client2, id, traceId) {
    super({
      client: client2,
      id,
      traceId,
...
async function flushTraces() {
  ...
  debug("Flushing Langfuse traces...");
...
function emitLLMCall(trace, index, total, llm, userText, toolStartTimes) {
  ...
  if (thinking)
    output.thinking = truncate(thinking);
  ...
  const gen = trace.generation({
    ...
    metadata: {
      ...
      ...llm.usage.cache_read_input_tokens ? { cache_read_input_tokens: llm.usage.cache_read_input_tokens } : {},
      ...llm.usage.cache_creation_input_tokens ? { cache_creation_input_tokens: llm.usage.cache_creation_input_tokens } : {},
...
function tracePendingSubagents(options) {
  ...
  for (const subagent of pendingSubagents) {
    try {
      ...
      debug(`Processing subagent ${toolName} (${subagent.agent_id})`);
      if (taskRunInfo?.deferred) {
        const def = taskRunInfo.deferred;
        parentTrace.span({
          id: taskRunInfo.observation_id,
          name: `Tool: Agent (${toolName})`,
          input: def.tool_input,
          output: def.tool_output,
          startTime: new Date(def.start_time),
          endTime: new Date(def.end_time),
...
// dist/scoring/match.js
var FEEDBACK_COMMAND_RE = /^\s*\/(?:langfuse-tracing:)?(?:feedback|journey)(?:\s|$)/;
function isFeedbackCommand(prompt) {
...
async function main() {
  ...
  if (messages.length === 0) {
    ...
    if (sessionState.current_trace_id) {
      await atomicUpdateState(config.stateFilePath, (s) => {
        ...
        return { ...s, [input.session_id]: { ...ss, current_trace_id: void 0 } };
      });
    }
    return;
  }
  if (!sessionState.current_trace_id) {
    debug(`No current_trace_id \u2014 likely a feedback turn. Advancing last_line ${sessionState.last_line} \u2192 ${lastLine} and exiting.`);
    await atomicUpdateState(config.stateFilePath, (s) => {
      const ss = getSessionState(s, input.session_id);
      return {
        ...s,
        [input.session_id]: { ...ss, last_line: lastLine, updated: (/* @__PURE__ */ new Date()).toISOString() }
      };
    });
    return;
  }
  ...
  if (tracedTurns > 0 && currentTraceId) {
    const lastTurn = turns[turns.length - 1];
    const userText = typeof lastTurn.userContent === "string" ? lastTurn.userContent : extractText(lastTurn.userContent);
    if (!isFeedbackCommand(userText)) {
      promoteTraceId = currentTraceId;
    } else {
      debug("Last turn looked like a feedback command (defensive check); not promoting last_substantive_trace_id");
    }
  }
